...
class ass_style():
    def __init__(self, setting_path: str = './setting/') -> None:
        ...
        # Reading hex color code
        self.coler_in_hex: dict[str, str] = load_setting(
            'css3_to_hex.json',
            setting_path
            )
    ...
    def __compse_info(self) -> str:
        ...
        # # Delete used keys from dictionaly, so it can format with loop
        # del tmp_dict["Head"]
        # del tmp_dict["msg"]

        # # Setting rest of "Sctipt Info" block
    ...
    def get_lang_code(self, lang_code: str) -> str:
        """Convert SMI language code to ASS language code

        Args:
            lang_code (str): SMI language code in all upper case
        ...
        try:
            return self.lan_code[lang_code]
        except:
            print(f"\"{lang_code}\" is not found, plase add to \"lan_code.json\".")
            return self.lan_code["UNKNOWNCC"]

    def get_color_code(self, str_color: str) -> str:
        return self.coler_in_hex[str_color]
    ...
    def compose_ass_styles(self) -> str:
...
if __name__ == '__main__':
    ...
    with open('./tmp_out.ass', 'w') as f:
        f.write(tmp.compose_ass_styles())
